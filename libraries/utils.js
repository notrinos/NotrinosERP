...
JsHttpRequest._request = function(trigger, form, tout, retry) {
	if (trigger.tagName=='A') {
		...
		if (_csrfTokenEl)
			content['_token'] = _csrfTokenEl.value;
	}
	else {
	...
	JsHttpRequest.query(
		...
		function(result, errors) {
			...
			if (result) {
				...
				if (errors.length>0)
					window.scrollTo(0,0);
				//document.getElementById('msgbox').scrollI/ntoView(true);
...
function move_focus(dir, e0, neighbours) {
	...
	for(var i=0; i<neighbours.length; i++) {
		var e = neighbours[i];
		var p = element_pos(e);
		if (p!=null && (e.className=='menu_option' || e.className=='printlink' || e.className == 'repclass_link' || e.className == 'repopts_link')) {
