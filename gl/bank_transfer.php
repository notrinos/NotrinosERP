...
function gl_payment_controls($trans_no) {
	...
	ref_row(_('Reference:'), 'ref', '', $Refs->get_next(ST_BANKTRANSFER, null, get_post('DatePaid')), false, ST_BANKTRANSFER, array('date' => get_post('DatePaid')));
...
function check_valid_entries($trans_no) {
	...
	if ($problemTransaction != null	) {
		if (!array_key_exists('trans_no', $problemTransaction))
			display_error(sprintf(_("This bank transfer change would result in exceeding authorized overdraft limit (%s) of the account '%s'"), price_format(-$problemTransaction['amount']), $problemTransaction['bank_account_name']
			));
		else
			display_error(sprintf(_("This bank transfer change would result in exceeding authorized overdraft limit on '%s' for transaction: %s #%s on %s."), $problemTransaction['bank_account_name'], $systypes_array[$problemTransaction['type']], $problemTransaction['trans_no'], sql2date($problemTransaction['trans_date'])));
		set_focus('amount');
		return false;
	}
	else {
		if (null != ($problemTransaction = check_bank_account_history(-$amnt_tr, $_POST['FromBankAccount'], $_POST['DatePaid']))) {
			if (!array_key_exists('trans_no', $problemTransaction))
				display_error(sprintf(_('This bank transfer would result in exceeding authorized overdraft limit of the account (%s)'), price_format(-$problemTransaction['amount'])));
			else
				display_error(sprintf(_('This bank transfer would result in exceeding authorized overdraft limit for transaction: %s #%s on %s.'), $systypes_array[$problemTransaction['type']], $problemTransaction['trans_no'], sql2date($problemTransaction['trans_date'])));
			set_focus('amount');
			return false;
		}
	}
...
function bank_transfer_handle_submit() {
	...
	if ($trans_no)
		$trans_no = update_bank_transfer($trans_no, $_POST['FromBankAccount'], $_POST['ToBankAccount'], $_POST['DatePaid'],	input_num('amount'), $_POST['ref'], $_POST['memo_'], $_POST['dimension_id'], $_POST['dimension2_id'], input_num('charge'), input_num('target_amount'));
	else {
		new_doc_date($_POST['DatePaid']);
		$trans_no = add_bank_transfer($_POST['FromBankAccount'], $_POST['ToBankAccount'], $_POST['DatePaid'], input_num('amount'), $_POST['ref'], $_POST['memo_'], $_POST['dimension_id'], $_POST['dimension2_id'], input_num('charge'), input_num('target_amount'));
	}
