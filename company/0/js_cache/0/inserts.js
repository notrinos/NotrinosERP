...
else if(string_contains(this.className,'combo2')){this.style.display='none';select.style.display='inline';setFocus(select);}
return false;};e.onkeyup=function(ev){var select=document.getElementsByName(this.getAttribute('rel'))[0];if(select&&select.selectedIndex>=0){var len=select.length;var byid=string_contains(this.className,'combo')||string_contains(this.className,'combo3');var ac=this.value.toUpperCase();var values=[];select.options[select.selectedIndex].selected=false;for(i=0;i < len;i++){values.push(select.options[i].value.toUpperCase());}
var matched=stringSimilarity.findBestMatch(ac,values);select.options[matched.bestMatchIndex].selected=true;$(select).select2().val(select.options[matched.bestMatchIndex].value);}
};e.onkeydown=function(ev){ev=ev||window.event;key=ev.keyCode||ev.which;if(key==13||!key||key===undefined){setTimeout(function(){e.blur();return false;},0);}
}
}
...
function _set_combo_select(e){e.setAttribute('_last',e.selectedIndex);e.onblur=function(){var box=document.getElementsByName(this.getAttribute('rel'))[0];if((this.selectedIndex!=this.getAttribute('_last'))||((string_contains(this.className,'combo')||string_contains(this.className,'combo3'))&&_update_box(this)))
this.onchange();}
...
_update_box(s);if(s.selectedIndex>=0){var sname='_'+s.name+'_update';var update=document.getElementsByName(sname)[0];if(update)
...
if(box&&(key==32)&&(string_contains(this.className,'combo2'))){this.style.display='none';box.style.display='inline';box.value='';setFocus(box);return false;}
else{if(key==13&&!e.length)
return false;}
}
}
var _w;function callEditor(key){var el=document.getElementsByName(editors[key][1])[0];if(_w)
_w.close();var left=(screen.width-editors[key][2])/2;var top=(screen.height-editors[key][3])/2;_w=open(editors[key][0]+el.value+'&popup=1',"edit","scrollbars=yes,resizable=0,width="+editors[key][2]+",height="+editors[key][3]+",left="+left+",top="+top+",screenX="+left+",screenY="+top);if(_w.opener==null)
...
return date;if(user.datefmt==0||user.datefmt==3){day=dat[1];month=dat[0];year=dat[2];}
else if(user.datefmt==1||user.datefmt==4){day=dat[0];month=dat[1];year=dat[2];}
else{day=dat[2];month=dat[1];year=dat[0];}
...
day=cur[2];else
year=cur[2];}
if(user.datefmt<3){if(day<10)
day='0'+parseInt(day,10);if(month<10)
month='0'+parseInt(month,10);}
if(year<100)
year=year<60 ?(2000+parseInt(year,10)):(1900+parseInt(year,10));if(user.datefmt==0||user.datefmt==3)
...
_set_combo_input(e);else{if(e.type=='text'){e.onkeydown=function(ev){ev=ev||window.event;key=ev.keyCode||ev.which;if(key==13){if(e.className=='searchbox')
...
function(e){e.onclick=function(){if(validate(e)){save_focus(e);var asp=e.getAttribute('aspect');if(asp&&(asp.indexOf('process')!==-1))
...
'button[aspect=popup]':function(e){e.onclick=function(){if(_w)
_w.close();var left=(screen.width-800)/2;var top=(screen.height-600)/2;_w=open(document.location+'popup=1',"edit","Scrollbars=0,resizable=0,width=800,height=600, top="+top+",left="+left+",screenX="+left+",screenY="+top);if(_w.opener==null)
...
e.onclick=function(ev){if(_validate._processing&&_validate._modified&&!confirm(_validate._processing)){ev.returnValue=false;return false;}
if(_hotkeys.alt)
...
};function stopEv(ev){if(ev.preventDefault){ev.preventDefault();ev.stopPropagation();}
else{ev.returnValue=false;ev.cancelBubble=true;window.keycode=0;}
return false;}
...
el.click();else if(asp.indexOf('process')!==-1)
...
link.onclick();else
if(link.target=='_blank'){window.open(link.href,'','toolbar=no,scrollbar=no,resizable=yes,menubar=no,width=900,height=500');openWindow(link.href,'_blank');}
else
