...
if (($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') && check_csrf_token()) {
	...
	if (can_process($Mode == 'ADD_ITEM')) {
		if ($selected_id != -1) {
			...
			if ($_POST['password'] != '')
				update_user_password($selected_id, $_POST['login_id'], md5($_POST['password']));

			display_notification_centered(_('The selected user has been updated.'));
		} 
		else {
			add_user($_POST['login_id'], $_POST['real_name'], md5($_POST['password']), $_POST['phone'], $_POST['email'], $_POST['role_id'], $_POST['language'], $_POST['print_profile'], check_value('rep_popup'), $_POST['pos']);
			$id = db_insert_id();
