...
hrm_require_report_controller(891);
...
/**
 * Resolve one Loan Outstanding Report employee display at the report's fixed as-of instant.
 *
 * The loan SQL remains authoritative for cohort, filters, loan amounts, outstanding
 * balances, status and ordering and supplies the exact legacy employee_name fallback.
 ...
 * @return string
 */
function loan_outstanding_report_authoritative_name($row, $as_of) {
...
function print_loan_outstanding_report() {
    ...
    hrm_log_restricted_employee_projection('loan_outstanding_report');
    $as_of = hrm_person_worker_utc_now();

    while ($row = db_fetch($res)) {
        $rep->TextCol(0, 1, $row['loan_id']);
        $rep->TextCol(1, 2, $row['employee_id'].' '.loan_outstanding_report_authoritative_name($row, $as_of));
