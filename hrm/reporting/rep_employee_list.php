...
hrm_require_report_controller(886);
...
/**
 * Resolve one Employee List display name at the report's fixed as-of instant.
 *
 * The report SQL remains the authoritative legacy cohort/filter/order source.
 * Missing, denied, unavailable, ambiguous, tampered, or unlinked identity
 * evidence returns the exact legacy employee_name value. Only an accepted
 ...
 * @return string
 */
function employee_list_report_authoritative_name($row, $as_of) {
...
function print_employee_list_report() {
    ...
    hrm_log_restricted_employee_projection('employee_list_report');
    $as_of = hrm_person_worker_utc_now();

    while ($row = db_fetch($res)) {
        $rep->TextCol(0, 1, $row['employee_id']);
        $rep->TextCol(1, 2, employee_list_report_authoritative_name($row, $as_of));
