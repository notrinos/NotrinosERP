...
hrm_require_report_controller(885);
...
/**
 * Resolve one Leave Computation Report employee display at the report's fixed as-of instant.
 *
 * The shared get_leave_balances() result remains authoritative for cohort, filters,
 * leave values and ordering and supplies the exact legacy employee_name fallback.
 ...
 * @return string
 */
function leave_computation_report_authoritative_name($row, $as_of) {
...
function print_leave_computation_report() {
    ...
    hrm_log_restricted_employee_projection('leave_computation_report');
    ...
    while ($row = db_fetch($res)) {
        ...
        $rep->TextCol(1, 2, leave_computation_report_authoritative_name($row, $as_of));
